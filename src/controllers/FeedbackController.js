...
const predictFeedback = async (req, res, next) => {
    try {
        ...
        const response = await axios.post(
            "http://103.56.163.117:5000/api/v1/predict",
            predictRequestBody
        );

        if (
            response.data.status === "successfuly" &&
            ...
            return res.json({
                ...
                data: response.data.data.map((result) => ({
                    ...
                    content: feedbackArray.find((f) => f.feedback_id === result.id)
                        ?.content,
                    predict: result.predict,
                })),
            });
...
const getFeedbacks = async (req, res, next) => {
    try {
        ...
        switch (searchBy) {
            case "predict":
                searchCondition = [{ predict: { [Op.like]: `${search}` } }];
                break;
            case "content":
                searchCondition = [{ content: { [Op.like]: `%${search}%` } }];
                break;
            case "product":
                searchCondition = [
                    {
                ...
                break;
            case "email":
                searchCondition = [{ "$User.email$": { [Op.like]: `%${search}%` } }];
                break;
            default: // 'all'
                searchCondition = [
        ...
        const feedbacks = await Feedback.findAndCountAll({
            attributes: ["feedback_id", "content", "predict", "created_at"],
            include: [
                {
        ...
        return res
            .status(500)
            .send("Gặp lỗi khi tải dữ liệu feedback, vui lòng thử lại");
    }
};
...
let create = async (req, res, next) => {
    ...
    if (product_variant_id === undefined)
        return res.status(400).send(" product_variant_id not exists");
    ...
    try {
        let customer = await User.findOne({
            where: { user_id: customer_id, role_id: 2 },
        });
    ...
    try {
        ...
        if (productVariant == null)
            return res.status(400).send("Product Variant  not exists");
    } catch (err) {
        console.log(err);
        return res.status(500).send("Error");
    }
...
let update = async (req, res, next) => {
    ...
    if (feedback_id === undefined)
        return res.status(400).send(" feedback_id not exists");
    let rate = req.body.rate;
    if (rate === undefined) return res.status(400).send(" rate not exists");
    let content = req.body.content;
    if (content === undefined) return res.status(400).send(" content not exists");
...
let detail = async (req, res, next) => {
    let customer_id = req.token.customer_id;
    if (!customer_id)
        return res.status(400).send({ message: "Ivalid Access Token" });
...
let list = async (req, res, next) => {
    ...
    if (product_id === undefined)
        return res.status(400).send(" product_id not exists");
